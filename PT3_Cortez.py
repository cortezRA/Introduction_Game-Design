# Adding customized game music and sound effects
# Adding one more enemy sprite
# Turning player knockback to temporary invulnerability when hurt (damaged by enemy)

import pygame
from pygame import mixer

pygame.init()
mixer.init() #initialize mixer for sound effects in game
...
bg_music = pygame.mixer.Sound('sfx/cave_drip.mp3')
bg_music.set_volume(.75)
bg_music.play(-1, 0)
jump_sfx = pygame.mixer.Sound('sfx/whoosh.wav')
jump_sfx.set_volume(.25)
castorb_sfx = pygame.mixer.Sound('sfx/spellcast.wav')
castorb_sfx.set_volume(1)
hitorb_sfx = pygame.mixer.Sound('sfx/ice_hit.mp3')
castorb_sfx.set_volume(2)
hurt_sfx = pygame.mixer.Sound('sfx/hurt.wav')
dead_sfx = pygame.mixer.Sound('sfx/death.wav')
deadmon_sfx = pygame.mixer.Sound('sfx/death2.wav')
deadmon_sfx.set_volume(1.5)
    ...
    def move(self):
        ...
        if not(self.isDead):
            ...
            if self.speed > 0: #move to right
                if self.x < self.path[1]: #reaching right end path
                    self.x += self.speed
                else: #change direction to left
                    self.speed = self.speed * -1
                    self.x += self.speed
            else: #move to left
                if self.x > self.path[0]: #reaching left end path
                    self.x += self.speed
...
def windowUpdate():
    ...
    elif enemy1.health == 0 and enemy2.health == 0:
...
play = player(600,420,260,220) #instance of player class
enemy1 = enemy(50,420,260,220,1150) #instance of enemy class
enemy2 = enemy(900,420,260,220,50)
bullets = []
run = True
...
while run:
    ...
    for bullet in bullets:
        ...
        if bullet.x+16 >= enemy1.hitSize[0] and bullet.x+16 <= enemy1.hitSize[0] + enemy1.hitSize[2]:
            if bullet.y+16 >= enemy1.hitSize[1] and bullet.y+16 <= enemy1.hitSize[1] + enemy1.hitSize[3]:
                ...
                if enemy1.health == 0:
                    enemy1.isDead = True
                    deadmon_sfx.play()
        elif bullet.x+16 >= enemy2.hitSize[0] and bullet.x+16 <= enemy2.hitSize[0] + enemy2.hitSize[2]:
            if bullet.y+16 >= enemy2.hitSize[1] and bullet.y+16 <= enemy2.hitSize[1] + enemy2.hitSize[3]:
                bullets.pop(bullets.index(bullet))
                score += 1
                enemy2.hit()
                hitorb_sfx.play()
    ...
    if play.hitSize[0]+50 >= enemy1.hitSize[0] and play.hitSize[0]+50 <= enemy1.hitSize[0] + enemy1.hitSize[2]:
        if play.hitSize[1]+50 >= enemy1.hitSize[1] and play.hitSize[1]+50 <= enemy1.hitSize[1] + enemy1.hitSize[3]:
            play.hit()
    ...
    if play.hitSize[0]+50 >= enemy2.hitSize[0] and play.hitSize[0]+50 <= enemy2.hitSize[0] + enemy2.hitSize[2]:
        if play.hitSize[1]+50 >= enemy2.hitSize[1] and play.hitSize[1]+50 <= enemy2.hitSize[1] + enemy2.hitSize[3]:
            play.hit()
    ...
    if not(play.isDead):
        ...
        if (keys[pygame.K_d] and play.x < WINDOW_SIZE - 180):
            play.x += play.speed
            play.left = False
            play.right = True
        ...
        elif (keys[pygame.K_a] and play.x > -80):
            play.x -= play.speed
            play.left = True
            play.right = False
            play.isStanding = False
